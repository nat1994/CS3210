...
void *gaussian_blur(void *ptr)
{
    ...
    struct colour* RGB;
    RGB = (struct colour*) ptr;

    unsigned char *src = RGB->src;
    float *dst = RGB->dst;
    int width = RGB->width;
    int height = RGB->height;
    float sigma = RGB->sigma;
    int ksize = RGB->ksize;
...
int main(int argc, char ** argv)
{
    ...
    pthread_create(&thread1, NULL, gaussian_blur, &red);
    pthread_create(&thread2, NULL, gaussian_blur, &blue);
    pthread_create(&thread3, NULL, gaussian_blur, &green);
